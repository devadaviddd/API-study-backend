...
import { SignUpDto, Password, Email } from '../domains';
import {PhoneNumber} from "../domains/models/phoneNumber";
import {Username} from "../domains/models/username";
  ...
  async execute(input: SignUpUseCaseInput): Promise<string> {
    try {
      ...
      if(isNotValidPhoneNumber) {
        throw new BadRequestionException('Number phone is not valid');
      }
